module.exports = {
    ...
    async execute(sock, m, dbs, args) {
        ...
        dbs.dbUsuarios.prepare('INSERT OR IGNORE INTO muted_users (jid, groupId) VALUES (?, ?)')
            .run(mention, jid);
        
        const msg = await sock.sendMessage(jid, { text: `🔇 Mutado: @${mention.split('@')[0]}`, mentions: [mention] }, { quoted: m });
        setTimeout(() => { sock.sendMessage(jid, { delete: msg.key }).catch(e=>e); }, 60000);
    }
};